...
const PrayerItem = ({ name, time, isNext }) => {
    ...
    // We use that key to lookup the translation: t(`prayers.${name}`)
    ...
    return (
        <View style={[
            ...
            <View style={[styles.content, isRTL && styles.rtlContent]}>
                <Text style={[styles.name, isNext && styles.highlightText]}>{displayName}</Text>
                <Text style={[styles.time, isNext && styles.highlightText]}>{formatTo12Hour(time)}</Text>
            </View>
            {isNext && (
